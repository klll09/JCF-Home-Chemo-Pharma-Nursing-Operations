...
import {
  ...
  PlusCircle, FileText, Search, Bell, ChevronDown, LogOut, User
} from "lucide-react";
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="relative" ref={profileRef}>
            <button onClick={() => setShowProfile(p => !p)}
              ...
              <div className="text-left">
                <p className="text-xs font-semibold text-gray-700">{currentUser?.name || "Admin"}</p>
                <p className="text-[10px] text-gray-400">{currentUser?.roles?.name || "Operations"}</p>
              </div>
              <ChevronDown size={13} className="text-gray-400" />
            </button>

            {showProfile && (
              <div className="absolute right-0 top-12 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-50">
                <div className="px-4 py-3 border-b border-gray-100">
                  <p className="text-sm font-semibold text-gray-800">{currentUser?.name || "Admin"}</p>
                  <p className="text-xs text-gray-400">{currentUser?.email || "admin@jcf.com"}</p>
                  <span className="inline-block mt-1 text-[10px] font-semibold bg-teal-100 text-teal-700 px-2 py-0.5 rounded-full">
                    {currentUser?.roles?.name || "Admin"}
                  </span>
                </div>
                <div className="py-1">
                  <button className="w-full flex items-center gap-2.5 px-4 py-2 text-xs text-gray-600 hover:bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-6 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { icon: Activity, label: "Active Sessions", value: stats.sessions, sub: "Currently in progress", light: "bg-emerald-50 text-emerald-600" },
            { icon: ClipboardList, label: "Pending Sign-off", value: stats.pending, sub: "Awaiting doctor approval", light: "bg-amber-50 text-amber-600" },
            { icon: Users, label: "Ready to Assign", value: stats.ready, sub: "Nurse not yet assigned", light: "bg-teal-50 text-teal-600" },
            { icon: UserCheck, label: "Available Nurses", value: stats.nurses, sub: "Ready for deployment", light: "bg-indigo-50 text-indigo-600" },
          ].map(({ icon: Icon, label, value, sub, light }) => (
            ...
            <div className="grid grid-cols-12 px-5 py-2.5 bg-gray-50 border-b border-gray-100">
              {["Patient", "Diagnosis", "Doctor", "Risk", "Status", ""].map((h, i) => (
                <span key={i} className={`text-[10px] font-semibold text-gray-400 uppercase tracking-wider
                  ${i === 0 ? "col-span-3" : i === 1 ? "col-span-3" : i === 2 ? "col-span-2" : i === 3 ? "col-span-1" : i === 4 ? "col-span-2" : "col-span-1"}`}>
                  {h}
                </span>
              ))}
            </div>
