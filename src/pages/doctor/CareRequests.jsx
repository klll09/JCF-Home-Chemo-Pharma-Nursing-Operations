...
import {
  ...
  ChevronRight, Calendar, Clock, MapPin, Stethoscope,
  PlusCircle
} from "lucide-react";
...
export default function DoctorCareRequests() {
  ...
  async function fetchRequests(doctorId) {
    if (!doctorId) return;
  ...
  async function fetchPatients(doctorId) {
    ...
    const { data } = await supabase
      .from("patients")
      .select("id, name, patient_code, address")
  ...
  const handlePatientChange = (e) => {
    ...
    // Auto-fill location from patient address if available
    const patient = patients.find(p => p.id === patientId);
    setForm(prev => ({
      ...prev,
      patient_id: patientId,
      location: patient?.address ? prev.location || patient.address : prev.location,
    }));
    setFormError("");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!profile?.id) return setFormError("Doctor profile not loaded. Please refresh.");
    ...
    // Reset form
    setForm({
      patient_id: "", service_type: "Home chemo",
      scheduled_date: "", duration: "2hr", location: "",
      required_nurse_skill: "", medicine_required: false,
    });
    setSuccess(true);
  ...
  const handleApprove = async (id) => {
    ...
    const { error } = await supabase
      ...
      .eq("id", id);
    setApproving(null);
    if (!error) await fetchRequests(profile?.id);
  };
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-3.5 flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-xs text-gray-400">Manage and create care sessions for your patients</p>
        </div>
        <button
          onClick={() => {
            setLoading(true);
            fetchRequests(profile?.id).finally(() => setLoading(false));
          }}
      ...
      <div className="flex-1 flex gap-5 p-6 overflow-hidden">
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-200 flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-gray-800">My Care Requests</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-2">
                ...
                <p className="text-xs text-gray-300">Create one using the form →</p>
              </div>
            ) : requests.map((r, idx) => (
              <div
                key={r.id}
                ...
                <div className="col-span-2 flex items-center">
                  {r.status === "Draft" && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1 bg-teal-600 hover:bg-teal-700 text-white text-[10px] font-semibold rounded-md transition disabled:opacity-60"
                    >
                ...
                {selected?.id === r.id && (
                  <div className="col-span-12 mt-2 pt-3 border-t border-teal-100">
                    <div className="grid grid-cols-4 gap-3">
                      {[
                        { label: "Diagnosis", value: r.patients?.diagnosis },
                        { label: "Location", value: r.location },
                        { label: "Duration", value: r.duration },
                        { label: "Medicine Required", value: r.medicine_required ? "Yes" : "No" },
                      ].map(({ label, value }) => (
        ...
        <div className="w-80 bg-white rounded-xl border border-gray-200 flex flex-col overflow-hidden shrink-0">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Create a session for your patient</p>
          </div>

          <div className="flex-1 overflow-y-auto px-5 py-4">
            <form onSubmit={handleSubmit} className="flex flex-col gap-3.5" noValidate>

              {/* Patient — only doctor's patients */}
              <div>
                ...
                {patients.length === 0 && !loading && (
                  <p className="text-[10px] text-gray-400 mt-1">No patients assigned to you yet.</p>
                )}
              </div>

              {/* Doctor — locked to self, just display */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Doctor</label>
                <div className="relative">
                  <Stethoscope size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-teal-400 pointer-events-none" />
                  <div className="w-full text-xs border border-gray-200 rounded-lg pl-7 pr-3 py-2 bg-teal-50 text-teal-700 font-medium">
                    {profile?.name || "You (logged in doctor)"}
                  </div>
                </div>
                <p className="text-[10px] text-teal-500 mt-1">✓ Auto-set to your account</p>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  name="required_nurse_skill"
                  value={form.required_nurse_skill}
                  onChange={handleChange}
                  placeholder="e.g. IV cannulation, Oncology"
                  className="w-full text-xs border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-400 placeholder:text-gray-300 bg-gray-50"
                />
              </div>

              {/* Medicine Required toggle */}
